...
public class Day11Part1 {
    public static void main(String[] args) {
        Path path = Paths.get("src/advent_of_code/year_2023/test_data/day11part1.txt");
        String[] arr = null;
        try {
    ...
    private static long solve(String[] examples) {
        ...
        for(int i = 0; i < galaxies.size(); i++) {
            ...
            for(int j = i + 1; j < galaxies.size(); j++) {
                ...
                int iFlat = Math.abs(rowPreSum[c1[0]] -  rowPreSum[c2[0]]);
                int jFlat = Math.abs(colPreSum[c1[1]] -  colPreSum[c2[1]]);
                result += Math.abs(c1[0] - c2[0]) + Math.abs(c1[1] - c2[1]) + iFlat +jFlat ;
            }
        }
